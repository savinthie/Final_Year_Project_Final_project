...
function ExpensesPage() {
    ...
    useEffect(() => {
        ...
        setTotalExpense(newTotal - expenseValues.median_family_income); // edited
        console.log("Total Expense:", newTotal - expenseValues.median_family_income)// edited
    }, [expenseValues]);
    ...
    const chartData = {
        ...
        datasets: [
            {
                label: 'Estimated Household Finances',
                data: Object.values(expenseValues),
    ...
    const counterfactualChartData = counterfactuals ? counterfactuals.map((cf, index) => ({
        labels: Object.keys(expenseValues),
        datasets: [
            {
                label: `AI Suggestion ${index + 1}`,
                data: Object.keys(expenseValues).map(key => cf[key.toLowerCase()]),
    ...
    return (
        ...
        <div className="expenses-page">
                    ...
                    <table className="counterfactuals-table">
                        <thead>
                            <tr>
                                <th>Feature</th>
                                <th>Original ($)</th>
                                <th>AI Suggestion 1 ($)</th>
                                <th>AI Suggestion 2 ($)</th>
                                <th>AI Suggestion 3 ($)</th>
                            </tr>
                        </thead>
                        <tbody>
                            {Object.keys(expenseValues).map(key => (
            ...
            {counterfactualChartData.map((data, index) => (
                <div key={index} className="chart-container-counterfactual">
                    <Bar data={data} />
                </div>
            ))}
